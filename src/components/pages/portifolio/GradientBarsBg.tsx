...
export default function GradientStripsBG(
  ...
  const stylePreset = content.stylePreset ?? props.stylePreset ?? 'pink-sunset'
  const gradientColors = content.gradientColors ??
    props.gradientColors ?? ['#E76F51', '#F4A261', '#E9C46A', '#FAF3DD']
  ...
  const isRibbedLook =
    shape === 'flat' && safeMinHeight >= 99 && safeMaxHeight >= 99
        ...
        >
          <div
            style={{
              ...
              WebkitMaskImage: shouldUseMask ? featherMaskImage : undefined,
              maskImage: shouldUseMask ? featherMaskImage : undefined,
            }}
          >
            {Array.from({ length: safeNumBars }).map((_, index) => {
              const directionalIndex =
                direction === 'right-to-left' ? safeNumBars - 1 - index : index
              const height = calculateHeight(directionalIndex, safeNumBars)
              let baseAngle = 0
              ...
              const pulsePart =
                shouldRunMotion && animationStyle !== 'none'
                  ...
                  : ''
              const animation =
                [appearPart, pulsePart].filter(Boolean).join(', ') || 'none'

              const hueRotate = barT * safeHueDrift
              const edgeFactor = Math.abs(barT)
              const brightness = 1 + (edgeFactor * safeIntensityDrift) / 100
              const saturation = 1 + (edgeFactor * safeIntensityDrift) / 150
              const colorFilter =
                  ...
                  style={
                    {
                      ...
                      marginRight:
                        index < safeNumBars - 1 ? `-${seamOverlapPx}px` : 0,
  ...
  content: {
    ...
    controls: {
      ...
      gradientColors: {
        ...
        hidden: (value: any) => value?.stylePreset !== 'custom',
      },
      gradientFrom: {
  ...
  layout: {
    ...
    controls: {
      ...
      curveIntensity: {
        ...
        step: 0.1,
        displayStepper: true,
        hidden: (value: any) => value?.shape === 'flat',
      },
    },
  },
  styleGroup: {
    ...
    controls: {
      ...
      blendMode: {
        ...
        hidden: (value: any) => !(value?.enableBlend ?? value?.useBlend),
      },
      blendOpacity: {
        type: ControlType.Number,
        title: 'Blend Opacity',
      ...
      noiseOpacity: {
        ...
        step: 0.01,
        displayStepper: true,
        hidden: (value: any) => !value?.showNoise,
      },
    },
  },
  states: {
    ...
    controls: {
      ...
      animationStyle: {
        ...
        defaultValue: 'pulse',
        hidden: (value: any) => value?.enableStatic,
      },
      pulseStrength: {
        type: ControlType.Number,
        title: 'Pulse Amount',
